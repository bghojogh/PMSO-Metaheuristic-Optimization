...
y_max = x_max;
y = x;
ChromosomsNumEntered = input('Please enter number of Chromosoms: ');
...
ChromosomsNum = ChromosomsNumEntered;
...
for globalIteration = 1:GlobalIterationNum
    ...
    for i = 1:ChromosomsNum
        ...
        if globalIteration~=1  
            while 1
                % Random Selection:
                parent1_Chromosome = ChromosomeLoc(:, round(1 + (ChromosomsNum-1)*rand));
                parent1_fitness = ChromosomsFitness(round(1 + (ChromosomsNum-1)*rand));
                parent2_Chromosome = ChromosomeLoc(:, round(1 + (ChromosomsNum-1)*rand));
                parent2_fitness = ChromosomsFitness(round(1 + (ChromosomsNum-1)*rand));
